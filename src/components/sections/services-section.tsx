        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
          {services.map((service, index) => (
            <ScrollAnimation key={index} animationClass="animate-fade-in-up">
              <div className="service-card bg-background dark:bg-dark-background p-8 rounded-xl border border-border dark:border-dark-border shadow-lg hover:shadow-xl transition-shadow duration-300 h-full flex flex-col">
                <service.icon className="h-10 w-10 text-primary dark:text-primary-light mb-6" />
                <h3 className="text-xl font-semibold mb-3">
                  {t(service.title)}
                </h3>
                <p className="text-text-secondary dark:text-dark-text-secondary mb-6 text-sm flex-grow">
                ...
                <ul className="flex flex-wrap gap-2">
                  {service.tech.map((techItem) => (
                    <li
                      key={techItem}
                      className="bg-surface-variant dark:bg-dark-surface-variant px-3 py-1 rounded-full text-xs font-mono text-text-muted dark:text-dark-text-muted"
                    >
                      {techItem}
                    </li>
                  ))}
